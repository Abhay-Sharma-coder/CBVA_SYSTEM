...
/**
 * Demo auth: returns whichever user is logged in via JWT, or whichever seeded user
 * the role switcher cookie names as a fallback.
 */
export class DemoAuthProvider implements AuthProvider {
  async currentUser(): Promise<User | null> {
    const jar = await cookies();

    // 1. Check JWT Authentication Cookie
    const jwtCookie = jar.get(JWT_COOKIE_NAME)?.value;
    if (jwtCookie) {
      const payload = verifyJwt<JwtUserPayload>(jwtCookie);
      if (payload?.userId) {
        const [user] = await db()
          .select()
          .from(schema.users)
    ...
    return null;
    /* Legacy role-switcher code intentionally disabled: a session requires JWT. */
    /*
    // 2. Check Demo Role Switcher Cookie
